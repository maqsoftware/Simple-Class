...
std::vector<std::string> MenuContext::splitRegEx(const std::string input, const std::string regex) {
...
void MenuContext::showAnswer(std::string type, std::string header)
{
   /* if(_showAnswerLayer != NULL) {
	...
	auto label1 = CommonLabel::createWithTTF(header.c_str(), "res/lang/eng/NotoSans-Regular.ttf", 100);
	...
	if (type.compare("wordPairs") == 0) {
		...
		for (auto wordPair = _wordsList.begin(); wordPair != _wordsList.end(); wordPair++) {
			...
			auto label1 = CommonLabel::createWithTTF(wordPair->first.c_str(), "res/lang/eng/NotoSans-Regular.ttf", 100);
			if (wordPair->first.length() > 8) {
				label1 = CommonLabel::createWithTTF(wordPair->first.c_str(), "res/lang/eng/NotoSans-Regular.ttf", 80);
			}
			label1->setColor(Color3B(0, 0, 0));
			...
			auto label3 = CommonLabel::createWithTTF(wordPair->second.c_str(), "res/lang/eng/NotoSans-Regular.ttf", 100);
			if (wordPair->second.length() > 8) {
				label3 = CommonLabel::createWithTTF(wordPair->second.c_str(), "res/lang/eng/NotoSans-Regular.ttf", 80);
			}
	...
	else if (type.compare("Words") == 0)
	{
		...
		for (int index = 0; index < _listOfWords.size(); index++) {
			...
			auto label1 = CommonLabel::createWithTTF(_listOfWords.at(index).c_str(), "res/lang/eng/NotoSans-Regular.ttf", 100);
			label1->setColor(Color3B(0, 0, 0));
		...
		for (auto wordPair = _listOfInitialSyllableWords.begin(); wordPair != _listOfInitialSyllableWords.end(); wordPair++) {
			...
			if (wordPair->first.length() > 45) {
				...
				auto sentencelabel1 = CommonLabel::createWithTTF(str1.c_str(), "res/lang/eng/NotoSans-Regular.ttf", 75);
				...
				auto sentencelabel2 = CommonLabel::createWithTTF(secondStr.c_str(),"res/lang/eng/NotoSans-Regular.ttf", 75);
			...
			{
				auto sentencelabel1 = CommonLabel::createWithTTF(wordPair->first.c_str(), "res/lang/eng/NotoSans-Regular.ttf", 100);
				sentencelabel1->setColor(Color3B(0, 0, 0));
			...
			for (int i = 0; i < listOfAnswers.size(); i++) {
				...
				auto label3 = CommonLabel::createWithTTF(listOfAnswers.at(i).c_str(), "res/lang/eng/NotoSans-Regular.ttf", 100);
