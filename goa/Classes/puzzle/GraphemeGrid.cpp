...
bool GraphemeGrid::init(GLfloat width, GLfloat height, int numRows, int numCols, const std::string spriteName, const std::vector<std::vector<std::string>> graphemes, const std::string graphemeUnselectedBackground, const std::string graphemeSelectedBackground) {
    if (!Layer::init()) {
        return false;
    }
...
Rect GraphemeGrid::getGraphemeRect(const std::string alphabet) {
    for (int i = 0; i < _numRows; i++) {
        for (int j = 0; j < _numCols; j++) {
