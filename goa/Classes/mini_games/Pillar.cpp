...
void Pillar::newCake()
{
	...
	auto topLabel = Label::createWithTTF(_title, LangUtil::getInstance()->getFontFile(), 100);
	...
	_topLabel = Label::createWithTTF(_wordList.at(_num).c_str(), LangUtil::getInstance()->getFontFile(), 100);
	...
	if (_cakeTouchFlag && _cake != nullptr)
	{
		...
		auto check =_wordList.at(_num);
			if (std::find(_wordCorrect.begin(), _wordCorrect.end(),check) != _wordCorrect.end())
			{
				...
				auto labelCake = Label::createWithTTF(check.c_str(), LangUtil::getInstance()->getFontFile(), 100);
