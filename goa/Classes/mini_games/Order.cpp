...
//	_sortedList = { "1","2","3","4","5" };
	...
	for (short i = 0; i < _sortedList.size(); i++) {
		...
		if (menu->getCurrentLevel() > 6 && menu->getCurrentLevel() < 12) {
			...
			auto topLabel = CommonLabel::createWithTTF(randomList.at(i).c_str(), LangUtil::getInstance()->getFontFile(), 100);
...
void Order::checkUserSortList( std::vector<int> const& list)
{
	Size visibleSize = Director::getInstance()->getVisibleSize();
	int score = 0;
	bool helpFlag = false;
