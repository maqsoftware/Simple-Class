...
public interface LessonDao {
    ...
    Lesson getLessonBySeq(int seq);

    @Query("SELECT * FROM Lesson WHERE seq<=:seq AND concept in (:concepts)")
    Lesson[] getLessonsBelowSeqAndByConcept(int seq, List<Integer> concepts);
    ...
    DataSource.Factory<Integer, Lesson> lessonsBySeq();

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    Long insertLesson(Lesson lesson);

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    Long[] insertLessons(Lesson... lessons);
}
