...
public interface UserLessonDao {
    ...
    UserLesson getUserLessonByUserIdAndLessonId(Long userId, Long lessonId);

    @Query("SELECT * FROM UserLesson WHERE userId=:userId ORDER BY lessonId DESC LIMIT 1")
    UserLesson getUserLessonByUserIdWithMax(Long userId);

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    Long insertUserLesson(UserLesson userLesson);

    @Update
    void updateUserLesson(UserLesson userLesson);
}
